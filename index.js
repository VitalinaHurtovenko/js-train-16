...
function filterStudentsByGrade(students, grade) {
  ...
  let studentsList = students.filter((student) => {
...
function logArrayElements(arr) {
  ...
  console.assert(Array.isArray(arr), "Аргумент має бути масивом!", null);
...
function warnAboutMaxArrayLength(arr, maxLength) {
  ...
  // Повертаємо null, що свідчить про неможливість обробки вхідних даних.
  console.assert(
    Array.isArray(arr) && typeof maxLength === "number",
...
function buildMatrix(size) {
  ...
  // Повертаємо null, що свідчить про неможливість обробки вхідних даних.
  console.assert(typeof size === "number", "Аргумент має бути числом!", null);
  ...
  for (let i = 0; i < size; i++) {
    ...
    for (let i = 0; i < size; i++) {
      ...
      newArrString.push(Math.floor(Math.random() * 10));
    }

    // Додаємо рядок до матриці.
    newArr.push(newArrString);
  }
